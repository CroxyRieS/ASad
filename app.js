require('dotenv').config();
  ...
  app.get("/robots.txt", function(req,res) {
    ...
    res.send(`Sitemap: https://thykescode.cf/sitemap.xml`);
});

app.get("/sitemap.xml", async function(req,res) {
    let link = "<url><loc>https://thykescode.cf/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
        link += "\n<url><loc>https://thykescode.cf/bot/"+bot.botID+"</loc></url>";
    })
  ...
  const renderTemplate = (res, req, template, data = {}) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](http://thykes.net//user/${a.id}) isimli kullanıcı **siteye** giriş yapmaya çalıştı fakat siteden engellendiği için giriş yapamadı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](http://thykes.net///user/${a.id}) isimli kullanıcı **siteye** giriş yaptı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/", checkMaintence, async (req, res) => {
      ...
      const arrayOfPremiums = require('./premium.json').premiums;
      console.log(arrayOfPremiums)
      renderTemplate(res, req, "index.ejs", {config, roles, botdata, getuser, arrayOfPremiums});
    });
    app.get("/dc", (req, res) => {
      res.redirect('https://discord.gg/SfbFhgXJE9');
    });  
    ...
    app.get("/code/:code", checkMaintence, checkAuth, async (req,res) => {
      ...
      if(kod == "javascript") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.javascript)) {
      ...
      } else return res.redirect("/error?code=403&message=You is not competent to do this.");
      }
      if(kod == "html") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.html)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "subs") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.altyapilar)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "5invites") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.besdavet)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "10invites") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.ondavet)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "15invites") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.onbesdavet)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "20invites") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.yirmidavet)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
      ...
      if(kod == "bdfd") {
      if(client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.bdfd)) {
        renderTemplate(res, req, "codeshare/codes/codelist.ejs", {
          req,
          roles,
          config,
    ...
    app.post("/code-request", checkMaintence, checkAuth, async (req,res) => {
       client.users.fetch(req.user.id).then(a => {
         ...
         client.channels.cache.get(channels.request).send(new Discord.MessageEmbed()
                              ...
                              .setDescription(`User **[${a.username}#${a.discriminator}](http://thykes.net///user/${a.id})** requested the code named **${rBody['kodadi']}**.`)
    ...
    app.get("/codes/:type", checkMaintence, checkAuth, async(req,res) => {
      let docs = await codesSchema.findOne({ code: req.params.type })
      console.log(docs)
    ...
    app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.votes).send(`> \`\`⚙️\`\` **|** \`\`${botdata.username}\`\` adlı bota oy verdi!  \`\`${req.user.username}\`\`  **\`(${botdata.votes + 1} votes)\`**`)
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.botlog).send(`> \`\`🎉\`\` **|** \`\`${a.tag}\`\` adlı botunu sisteme eklenmiştir,lütfen bekleyiniz..(Geliştiren**:**<@${req.user.id}>)`)
      ...
      if(botdata.status != "Approved") {
        if(req.user.id == botdata.ownerID || botdata.coowners.includes(req.user.id)) {
          ...
          client.users.fetch(req.params.botID).then(abot => {
              renderTemplate(res, req, "botlist/bot.ejs", { req, abot, config, botdata, coowner, aowner, roles, arrayOfPremiums});
          });
          });
        } else {
    ...
    app.get('/redirect/:type/:botId', (req, res) => {
      const { type } = req.params;
      if(type === "invite") {
        const { botId } = req.params;
        if(botId === "791027511731552256") {
          res.redirect("https://discord.com/api/oauth2/authorize?client_id=791027511731552256&permissions=3624266870&redirect_uri=https%3A%2F%2Fapi.syfac.xyz%2Fapi%2Fauth%2Fdiscord%2Fredirect&scope=applications.commands%20bot")
        } else {
          res.redirect(`https://discord.com/oauth2/authorize?client_id=${botId}&permissions=8&scope=bot`)
        }
      }
    })
    ...
    app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.botlog).send(`> \`\`🤖\`\` **|** \`\`${a.tag}\`\` adlı botunu düzenledi <@${req.user.id}>`)
      ...
      app.post("/certification/apply", checkMaintence, checkAuth, async (req, res) => {
          ...
          client.channels.cache.get(channels.botlog).send(`> \`\`📓\`\` **|** \`\`${botdata.username}\`\` adlı botu için bir sertifika istedi **<@${botdata.ownerID}>**.`)
      });
      //
      ...
      app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         client.users.fetch(req.params.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`> \`\`🎉\`\` **|** \`\`${bota.tag}\`\` adlı botun **<@${req.user.id}>** tarafından onaylandı**!** `)
              client.users.cache.get(botdata.ownerID).send(`**${bota.tag}** adlı botunuz onaylandı.`)
         });
      ...
      app.get("/admin/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        console.log(guild.members.cache.get(botdata.botID))
          try { guild.members.cache.get(botdata.botID).kick(); } catch (err) { console.log(err) }
     ...
     let botdata = await botsdata.findOne({ botID: req.params.botID });
           client.users.fetch(botdata.ownerID).then(sahip => {
               client.channels.cache.get(channels.botlog).send(`> \`\`⛔️\`\` **|**  **${botdata.username}** adlı botunuz <@${botdata.ownerID}> reddedildi. `)
       ...
       app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          ...
          client.users.fetch(botdata.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`> \`\`📌\`\` **|** \`\`${bota.tag}\`\` adlı bota **<@${botdata.ownerID}>** bir sertifika verildi.`)
       ...
       app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`Adlı kullanıcı **[${req.user.username}](http://thykes.net///user/${req.user.id})** adlı kodu ekledi **${rBody['codename']}** to the system.`)
          .addField("Code Link", `http://thykes.net///code/${kod}`, true)
       ...
       app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`Adlı kullanıcı **[${req.user.username}](http://thykes.net///user/${req.user.id})** adlı kodu düzenledi **${rBody['codename']}**.`)
          .addField("Code Link", `http://thykes.net///code/${kod}`, true)
          .addField("Code Description", rBody['codedesc'], true)
          .addField("Code Category", rBody['category'], true)
          )
      ...
      app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        .setAuthor("http://thykes.net//", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<a:online:833375738785824788> vCodes are **active** again!\n[Click to redirect website](http://thykes.net//)`)
    ...
    app.get("/user/:userID", checkMaintence, async (req, res) => {
      ...
      renderTemplate(res, req, "profile/profile.ejs", {member, req, roles, config, codecount, uptimecount, pdata, botdata, arrayOfPremiums});
      });
    });
